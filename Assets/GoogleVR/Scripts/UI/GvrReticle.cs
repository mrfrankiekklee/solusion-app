  ...
  public void OnGazeStay(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
    ...
    SetGazeTarget(intersectionPosition, isInteractive);
		if (gazedAt != null && gazeStartTime > 0f) { 
			if (Time.time - gazeStartTime > 3.0f && ExecuteEvents.CanHandleEvent<TimeInputHandler> (gazedAt)) {
				gazeStartTime = -1f;
				ExecuteEvents.Execute (gazedAt, null, (TimeInputHandler handler, BaseEventData data) => handler.HandleTimedInput ());
			}
		}
  }
