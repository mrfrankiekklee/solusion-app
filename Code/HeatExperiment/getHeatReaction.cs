...
public class getHeatReaction : MonoBehaviour {
	...
	void handleHeatReaction(){
		...
		else if (call.getheatReaction() == "white explosion") {
			heatReaction = Instantiate(Resources.Load("WhiteExplosion") as GameObject, new Vector3(-6f,9f,-23f),transform.rotation);

		} 
		else if (call.getheatReaction() == "sparks") {
			heatReaction = Instantiate(Resources.Load("Flare") as GameObject, new Vector3(-6f,9f,-23f),transform.rotation);
